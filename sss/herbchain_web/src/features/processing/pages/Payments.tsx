      ...
      <Card className="overflow-hidden">
        ...
        <CardContent>
          <div className="flex items-center gap-0 overflow-x-auto pb-2">
            {['Collection Completed', '→', 'Processing Completed', '→', 'Payment Released', '→', 'Manufacturing', '→', 'Final Settlement'].map((step, i) => (
              step === '→' ? (
                <div key={i} className="text-muted-foreground font-bold px-2 text-lg">→</div>
              ) : (
                <div key={i} className={`flex-1 min-w-32 p-3 rounded-lg text-center text-xs font-medium border shrink-0 ${i === 2 || i === 4 ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400' : 'bg-muted/50 border-border text-muted-foreground'}`}>
                  {step}
                </div>
              )
            ))}
