...
export default function Payments() {
  ...
  const myPayments = payments.filter((p) => p.stage === 'Supply Chain' || p.recipientRole === 'Supply Chain');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader
        title="Payments"
        description="Record and track payments to manufacturers for delivered stock"
        actions={
          <Button size="sm" className="h-8 text-xs bg-cyan-600 hover:bg-cyan-700 text-white" onClick={() => setRecording(true)}>
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent>
          <div className="flex items-center gap-0 overflow-x-auto pb-2">
            {['Manufacturing Completed', '→', 'Dispatch Confirmed', '→', 'Delivery Completed', '→', 'Payment Released', '→', 'Final Settlement'].map((step, i) => (
              step === '→' ? (
                <div key={i} className="text-muted-foreground font-bold px-2 text-lg">→</div>
              ) : (
                <div key={i} className={`flex-1 min-w-32 p-3 rounded-lg text-center text-xs font-medium border shrink-0 ${i === 4 || i === 6 ? 'bg-cyan-50 dark:bg-cyan-950/30 border-cyan-200 dark:border-cyan-800 text-cyan-700 dark:text-cyan-400' : 'bg-muted/50 border-border text-muted-foreground'}`}>
                  {step}
                </div>
              )
            ))}
            ...
            <TableBody>
              {myPayments.map((p) => (
                <TableRow key={p.id} className="table-row-hover">
                  ...
                  <TableCell className="text-sm font-bold text-cyan-700 dark:text-cyan-400">₹{p.amount.toLocaleString('en-IN')}</TableCell>
      ...
      {recording && (
        <RecordPaymentDialog stage="Supply Chain" defaultRecipientRole="Manufacturer" onClose={() => setRecording(false)} />
      )}
    </div>
  );
}
