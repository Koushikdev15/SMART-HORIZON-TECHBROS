...
export default function Payments() {
  ...
  const myPayments = payments.filter((p) => p.stage === 'Manufacturing' || p.recipientRole === 'Manufacturer');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader
        title="Payments"
        description="Record and track payments to processing units for certified batches"
        actions={
          <Button size="sm" className="h-8 text-xs bg-blue-600 hover:bg-blue-700 text-white" onClick={() => setRecording(true)}>
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent>
          <div className="flex items-center gap-0 overflow-x-auto pb-2">
            {['Lab Certified', '→', 'Manufacturing Completed', '→', 'Payment Released', '→', 'Supply Chain', '→', 'Final Settlement'].map((step, i) => (
              step === '→' ? (
                <div key={i} className="text-muted-foreground font-bold px-2 text-lg">→</div>
              ) : (
                <div key={i} className={`flex-1 min-w-32 p-3 rounded-lg text-center text-xs font-medium border shrink-0 ${i === 2 || i === 4 ? 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400' : 'bg-muted/50 border-border text-muted-foreground'}`}>
                  {step}
                </div>
              )
            ))}
            ...
            <TableBody>
              {myPayments.map((p) => (
                <TableRow key={p.id} className="table-row-hover">
                  ...
                  <TableCell className="text-sm font-bold text-blue-700 dark:text-blue-400">₹{p.amount.toLocaleString('en-IN')}</TableCell>
      ...
      {recording && (
        <RecordPaymentDialog stage="Manufacturing" defaultRecipientRole="Processing & Laboratory" onClose={() => setRecording(false)} />
      )}
    </div>
  );
}
